...
Dynamic ACLs are used to enforce push permissions: they make the decisions on who can push to which branches on which repositories.
...
After checking whether the user has access to the repository, it will call to either Git or <<repoBridge>> to provide the actual Git operations to the client.
...
This section gives background as to how the various components interact with eachother during various flows.


=== Web interface and Workers

The Pagure web interface and workers interacts with repositories via `python-libgit2`, which is a CPython extension making the `LibGit2` library available from Python.
...
NOTE: This lookup is currently not as efficient as could be, the changes needed to do that are documented and in the queue.


==== Repo ACL Checking

The KeyHelper will tell sshd to restrict the users' SSH access as much as possible (it will provide the `restrict` option, which enabled all of the limitations SSH implements at any time), and use an entry command that points to <<AclChecker>>.
...
If the username passed through by KeyChecker does not have access to the requested repository, it will print an error and `exit(1)`.
If it determines that the user does have access to the request repository, it determines whether the project's repositories are on repoSpanner.
...
It will also provide the repoSpanner region in an environment variable so the repoBridge can look up the corresponding
...
This section contains information on what will/should fail with various error conditions, and where to look at for debugging purposes.
Ideally, sections contain example log entries for various sides to help ease finding the correct section.
This section is expected to be expanded as operational experience is gathered.


=== repoSpanner nodes offline

A repoSpanner region has a specific number of nodes.
The system is designed so that as long as a majority of nodes keep communication with eachother, everything will proceed in an orderly fashion.
...
If any node was offline while a push was processed, it will be brought back in sync with regards to both refs and actual Git objects after it reestablishes communication.

If a majority of the nodes go offline, or more than one network partitioning occurs leading to no partition with a majority of nodes online, all of the nodes will become readonly, and any attempts to push to them will error out with an "No majority of nodes online" error.
